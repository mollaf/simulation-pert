...
for (var i = 0; i < random_number_count; i++) {
    exp_array1 += `${rv1.value[i]} &  ${exp1.value[i]} \\approx ${Math.round(exp1.value[i])} \\\\ \n`
}
...
for (var i = 0; i < random_number_count; i++) {
    exp_array2 += `${rv2.value[i]} &  ${exp2.value[i]} \\approx ${Math.round(exp2.value[i])} \\\\ \n`
}
...
for (var i = 0; i < random_number_count; i++) {
    exp_array3 += `${rv3.value[i]}  & ${rv4.value[i]} &  ${normal1.value[i]}  \\approx ${Math.round(normal1.value[i])} \\\\ \n`
}
...
for (var i = 0; i < random_number_count; i++) {
    exp_array4 += `${rv5.value[i]}  & ${rv6.value[i]} &  ${normal2.value[i]} \\approx ${Math.round(normal2.value[i])} \\\\ \n`
}
...
const calculate_pert = (graph_original) => {

    const graph = graph_original.map(a => Object.assign({}, a) )
